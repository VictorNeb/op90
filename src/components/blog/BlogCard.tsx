...
import { ArrowUpRight } from 'lucide-react';interface BlogCardProps {
  post: NotionPost;
}export function BlogCard({ post }: BlogCardProps) {
  const featuredImage = post.thumbnail;  // Ensure the content preview is a string
  const contentPreview = post.content && typeof post.content === 'string' 
    ? post.content.split('\n\n')[0] 
    : 'No content available';  return (
